...
at \url{https://github.com/RyoMurakami-MedRobo/utrasound-edu}.
\end{abstract}
...
\begin{enumerate}[leftmargin=*]
  ...
  offset from a quantized fractional one).
...
the mock backend's envelope peak fell at $+0.51$ samples and \must's at
$-0.49$ samples relative to the geometric expectation --- opposite signs, of
comparable magnitude, i.e.\ no systematic bias in either direction. This
confirms that $t_0 = 0$ (the convention both \code{sim\_engine} and the
\must{} adapter use) is correct for both backends.
...
The mean correlation across all seven cases is $0.916$ (range $0.894$--$0.940$);
it is lowest for the 128-element probe, the direction in which the mock's
missing diffraction and directivity terms would be expected to matter most.
The sub-sample lag is consistently close to $-1.0$ samples ($-50$\,ns at
$f_s=20$\,MHz) across every scheme and element count (mean $-1.02$, range
$-1.08$ to $-0.94$), i.e.\ the mock backend's echo arrives about one sample
ahead of \must's. Since $-50$\,ns is a quarter of one carrier period at
5\,MHz, this is well below anything that would perceptibly shift a
reconstructed image, and is consistent with the two backends using
different pulse-echo waveforms (a symmetric Gaussian-modulated sinusoid for
the mock vs.\ \must's simulated pulse-echo response, visibly less symmetric
in Fig.~\ref{fig:rf-compare}) rather than with a delay-law disagreement.
Figure~\ref{fig:rf-compare} (top) shows this directly: the two traces peak
at the same time and have comparable envelopes, but visibly different fine
structure, which is exactly the decomposition the correlation coefficient
and the lag measurement, together, are designed to separate.
...
with. The 16-element case also shows a substantial absolute difference
($2.04$ vs.\ $1.61$\,mm), but here the mainlobe is wide enough (spanning
$\sim$160 grid cells at the fine spacing) that the discrepancy is not a
grid artefact and instead reflects the same missing-diffraction effect
acting on a small, strongly diffraction-limited aperture.
...
\url{https://github.com/RyoMurakami-MedRobo/utrasound-edu} under the MIT
